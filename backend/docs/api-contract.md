...
**Status: frozen as of Aug 20. Unchanged as of Aug 21. Any change must be agreed by both of us and logged in section 8.**

**Note on repo location:** this file belongs at `docs/api-contract.md` in the shared repo. Neeraj currently has UI code at the repository root; once that moves to `frontend/`, this path holds.
...
- `2026-08-30` — **device geolocation for lab search (additive, optional).** `POST /api/episodes` now accepts optional `lat` and `lng` form fields (floats). When both are present, the logistics agent books the lab nearest to those exact coordinates; when absent, it falls back to the patient profile's city (unchanged behavior). Frontend TODO (Neeraj): request `navigator.geolocation` at episode creation and include `lat`/`lng` in the create FormData if permission is granted.
