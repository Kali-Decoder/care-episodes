...
- **FRONTEND LIVE on Firebase Hosting (Aug 24) — full stack now hosted on GCP.** Firebase had never been added to the project and Neeraj had (unknowingly) deployed to Vercel; added Firebase to the existing `care-episode-agent` GCP project (`firebase projects:addfirebase`, done by shekharshashank1211@gmail.com = project owner) and deployed the Next.js static export (`client/` → `out/`, 99 files) wired to the live backend. **Hosting URL: `https://care-episode-agent.web.app`** (submission's hosted URL; supersedes Vercel — Vercel PR check is now irrelevant, disconnect or ignore). Smoke-tested: `/welcome` 200, CORS to backend 200 (`allow-origin: *`). Note: build bakes in `NEXT_PUBLIC_API_BASE_URL` + `USE_MOCKS=false` at build time, so re-run `npm run build` + `firebase deploy` after any backend-URL change. GCP/Firebase/credits all on shekharshashank1211@gmail.com; git commits on shashank@frontier.ventures (Ever7est).
...
### Aug 24 — frontend live on Firebase Hosting
Discovered there was no Firebase project (Neeraj had deployed to Vercel unknowingly). Added Firebase to the existing care-episode-agent GCP project and deployed the built `client/out` static export wired to the live Cloud Run backend. Full stack is now hosted on GCP: UI at https://care-episode-agent.web.app, backend on Cloud Run, both in one project. Smoke-tested serving + CORS. This is the submission's hosted URL; Vercel is abandoned. Also merged agent→main (Neeraj) earlier today.
